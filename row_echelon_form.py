def multiply_row_with_scalar(row, scalar):
    'elementary operation which allows you to multiply a vector .i.e. vector row of matrix in this case with a scalar'
...
def to_echelon_form(dup_matrix, reduce_as_well=False):
    ...
    'a function which returns the echelon form of the given input matrix and reduces it as well if "reduce_as_well" is set to True'
    ...
    for j in range(0, len(dup_matrix[0])): # iterate over columns of the matrix
        for i in range(pivot[0] + 1, len(dup_matrix)): # skip rows which already have a pivot and iterate over the rest of the rows
            if pivot[1] != j: 
                ...
                if dup_matrix[i][j] != 0:
                    ...
                    print('After Multiplying row#%d with %s and adding it to row#%d we have' % (pivot[0] + 1, -dup_matrix[i][j], i + 1))
                    dup_matrix[i] = add_rows(dup_matrix[i],multiply_row_with_scalar(dup_matrix[pivot[0]], -dup_matrix[i][j]))
                    print_matrix(dup_matrix)
...
def create_matrix_of_random_size(min_size = 3, max_size = 6, min_element = 0, max_element = 99):
  ...
  'returns a matrix M of size (m,n) where min_size <= no. of rows or cols <= max_size and min_element <= element <= max_element'
